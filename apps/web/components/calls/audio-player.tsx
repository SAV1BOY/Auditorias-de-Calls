...
export function AudioPlayer({
  ...
  // External seek
  useEffect(() => {
    if (currentSeek !== undefined && audioRef.current) {
      audioRef.current.currentTime = currentSeek
    }
  }, [currentSeek])
        ...
        {pointMarkers.map((marker) => {
          ...
          return (
            <div
              key={marker.id}
              className={cn(
                "absolute top-1/2 -translate-y-1/2 h-2.5 w-2.5 rounded-full border-2 border-background z-10",
