          ...
          {comment.replies.map((reply) => (
            <div key={reply.id} className="space-y-1">
              <div className="flex items-center gap-2 text-sm">
                ...
                {reply.timestamp_sec !== null && (
                  <button
                    className="text-xs text-blue-600 hover:underline"
                    onClick={() => onSeek?.(reply.timestamp_sec!)}
                  >
                    {formatTimestamp(reply.timestamp_sec)}
                  </button>
                )}
              </div>
...
export function CommentsPanel({
  ...
  const refresh = useCallback(async () => {
    const data = await getComments(auditId)
    setComments(data)
    setLoading(false)
  }, [auditId])
  ...
  return (
    <div className="space-y-4">
      ...
      <CommentForm
        ...
        placeholder="Adicionar feedback no momento atual..."
        onSubmitted={refresh}
      />
      ...
      {comments.length === 0 ? (
        <div className="py-8 text-center text-muted-foreground">
          <CircleDot className="mx-auto h-10 w-10 mb-2 opacity-20" />
          <p>Nenhum comentário ainda.</p>
          <p className="text-sm mt-1">
            Adicione feedback em momentos específicos do áudio.
          </p>
        </div>
      ) : (
