...
const STATUS_COLORS: Record<string, string> = {
  ...
  completed: "text-muted-foreground bg-surface-interaction",
}
    ...
    <Card>
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          ...
          <span className={cn("rounded-full px-2 py-0.5 text-xs font-medium font-technical", STATUS_COLORS[competition.status] ?? "text-muted-foreground bg-surface-interaction")}>{STATUS_LABELS[competition.status] ?? competition.status}</span>
        </div>
        <div className="flex items-center gap-4 text-xs text-muted-foreground mt-2">
          ...
          <span className="font-technical">{METRIC_LABELS[competition.metric] ?? competition.metric}</span>
        </div>
      </CardHeader>
      <CardContent>
            ...
            {top3.map((standing) => (
              <div key={standing.closer_id} className="flex items-center justify-between rounded-md px-2 py-1.5">
                <div className="flex items-center gap-2">
                  {standing.rank === 1 ? <Trophy className="h-4 w-4 text-amber-400" /> : <Medal className={cn("h-4 w-4", standing.rank === 2 ? "text-muted-foreground" : "text-primary-dim")} />}
                  <span className="text-sm font-medium">{standing.closer_name}</span>
                </div>
                <span className="text-sm font-semibold font-technical tabular-nums">{standing.value}</span>
              </div>
            ))}
          </div>
        )}
