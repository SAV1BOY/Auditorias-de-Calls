...
const METRIC_LABELS: Record<string, string> = {
  score_avg: "Score Médio",
  score_min: "Score Mínimo",
  classificacao_count: "Calls não-Fracas",
  taxa_fechamento: "Taxa Fechamento",
  dimension_avg: "Dimensão Média",
}
...
export function GoalsDashboard({ goals, closers }: GoalsDashboardProps) {
  ...
  function handleStatusChange(goalId: string, status: "active" | "completed" | "failed") {
    startTransition(async () => {
      const result = await updateGoalStatus(goalId, status)
      if (result.error) {
        toast({
          title: "Erro",
    ...
    <div className="space-y-6">
      ...
      {completedGoals.length > 0 && (
        <div className="space-y-4">
          ...
          >
            {showCompleted ? "Ocultar" : "Mostrar"} Metas Concluídas ({completedGoals.length})
          </Button>
...
function GoalCard({
  ...
  isPending: boolean
  onStatusChange: (id: string, status: "active" | "completed" | "failed") => void
  ...
  const displayCurrent = isMetric
    ? `${goal.current_value}%`
    : goal.current_value.toFixed(1)
  const displayTarget = isMetric
    ? `${goal.target_value}%`
